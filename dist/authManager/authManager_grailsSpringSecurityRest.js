...
var GrailsSpringSecurityRestAuthManager = {
    ...
    isTokenValid: function ($http, $rootScope, $cookieStore) {
      ...
      $http({
        ...
        self.logger.info('back from validate check, valid: TRUE!', [
          data,
          status,
          headers,
          config
        ...
        self.logger.info('back from validate check, valid: FALSE!', [
          data,
          status,
          headers,
          config
    ...
    save: function (dataFromLoginPost, $rootScope, $cookieStore, headers) {
      ...
      $cookieStore.put('authToken', dataFromLoginPost[this.jsonResponseTokenName]);  /*  $cookieStore('JSESSIONID', auth_hash.JSESSIONID);
